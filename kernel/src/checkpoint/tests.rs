...
/// A table partitioned by a NON-NULLABLE column must checkpoint with a nullable
/// `partitionValues_parsed` field.
///
/// A checkpoint batch interleaves add rows (carrying partition values) with non-add rows
/// (protocol/metaData) whose `add.partitionValues_parsed` struct is null. If the parsed
/// partition schema copied the column's declared nullability, the non-nullable `category` field
/// would carry unmasked nulls -- which Arrow `StructArray` validation rejects with "Found
/// unmasked nulls for non-nullable StructArray field 'category'" on checkpoint read-back. The
/// parsed partition schema therefore forces its fields nullable (Spark's `schema.asNullable`).
/// This test asserts the emitted field is nullable and that the checkpoint round-trips.
/// `writeStatsAsStruct=true` is required for `partitionValues_parsed` to be emitted at all.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_checkpoint_with_non_nullable_partition_column() -> DeltaResult<()> {
    ...
    // Commit 0: protocol + metadata partitioned by a NON-NULLABLE column, writeStatsAsStruct on.
    let metadata = Action::Metadata(
        Metadata::try_new(
            ...
            StructType::new_unchecked([
                StructField::nullable("id", KernelDataType::LONG),
                StructField::not_null("category", KernelDataType::STRING),
            ])
            .into(),
            vec!["category".into()],
            0,
    ...
    write_commit_to_store(&store, vec![create_basic_protocol_action(), metadata], 0).await?;

    // Commit 1: an add carrying a partition value plus stats.
    let mut add = Add {
        path: "category=books/file1.parquet".into(),
        data_change: true,
        stats: Some(
    ...
    // Materialize the checkpoint batch and assert the emitted partitionValues_parsed.category
    // field is nullable. Without the fix this field mirrors the column's non-nullable declaration.
    let snapshot = Snapshot::builder_for(table_root).build(&engine)?;
    let writer = snapshot.clone().create_checkpoint_writer(&engine)?;
    let mut found_pv_parsed = false;
    for batch_result in writer.checkpoint_data(&engine)? {
        ...
        let add_col = record_batch
            ...
            .unwrap()
            .as_struct_opt()
            .expect("add column should be a struct");
        let pv_parsed = add_col
            .column_by_name("partitionValues_parsed")
        ...
        assert!(
            pv_parsed
                ...
                .expect("partitionValues_parsed should have a category field")
                .is_nullable(),
            "non-nullable partition column must be emitted as nullable in partitionValues_parsed"
        );
        found_pv_parsed = true;
    }
    assert!(
        found_pv_parsed,
        "checkpoint should emit an add.partitionValues_parsed column"
    );

    // The write path must succeed, and reading the table back exercises the parquet reader over
    // the written checkpoint (including its partitionValues_parsed column).
    snapshot.checkpoint(&engine, None)?;
    let reloaded = Snapshot::builder_for(Url::parse("memory:///")?).build(&engine)?;
